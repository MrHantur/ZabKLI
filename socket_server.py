...
def edit(id, day, idChange, changeTo, isChangedClassesUse):
    ...
    return str(
        f'Changed lesson number {idChange + 1} for day {day + 1} for class {id} to {changeTo}; now its {newString}')
...
def getLessons(id):
    ...
    for currentDay in range(7):  # Заполняем массив значениями, о том, изменены ли уроки
...
if __name__ == '__main__':
    host = '185.177.216.236'  # ip адрес сервера, на котором держится программа
    # host = "localhost"
    port = 20  # порт на котором работает программа
    ...
    olympiadsWeekNames = [[], ["Региональный этап ВСОШ по английскому языку"], [], [], [],
                          ["Заключительный этап ОРМО по физике"],
                          ["Заключительный этап Всесибирской олимпиады по биологии",
                           "Заключительный этап ОРМО по русскому языку",
                           "Заключительный этап Формулы Единства по физике"]]
    dynamicOlympiadsNames = ["Конкурс журнала Квантик по математике"]
    olympiadsWeekDescription = [[], ["Вторник, Бутина 65"], [], [], [], ["Суббота, гимназия №4"],
                                ["Воскресенье, с 12:00", "Воскресенье, гимназия №4", "Воскресенье, с 12:00"]]
